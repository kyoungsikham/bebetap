...
class NewbornWidget : GlanceAppWidget() {
    override suspend fun provideGlance(context: Context, id: GlanceId) {
        ...
        provideContent {
            Column(
                ...
                Text("BebeTap 신생아", style = TextStyle(fontSize = 10.sp, color = ColorProvider(WidgetColors.Gray), fontWeight = FontWeight.Medium))
                ...
                // 모유
                Row(verticalAlignment = Alignment.Vertical.CenterVertically) {
                    Text("❤️ 모유  ", style = TextStyle(fontSize = 12.sp, color = ColorProvider(WidgetColors.Gray)))
                    Text(elapsedLabel(store.lastBreastTime), style = TextStyle(fontSize = 12.sp, fontWeight = FontWeight.Bold))
                }
                Spacer(GlanceModifier.height(5.dp))
...
class NewbornWidgetReceiver : GlanceAppWidgetReceiver() {
    override val glanceAppWidget = NewbornWidget()
}
